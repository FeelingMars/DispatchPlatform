...
{
    public partial class frmDispatchLog : frmBase
    {
        private bool _showMsgbox = false;//第一次打开时不显示找不到的对话框
        private string _tableName = "DispatchLog";

        public frmDispatchLog()
        {
            InitializeComponent();
            this.FormTitle = "调度日志";
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string strW=" dt_datetime>='" + dtTelStart.Value + "' and  dt_datetime<='" + dtTelEnd.Value + "' order by dt_datetime desc";
                    dgvLog.DataSource = new DB_Talk.BLL.v_DispatchLog().GetList(strW).Tables[0];;
                        //.GetListEx
