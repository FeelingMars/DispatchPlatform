...
namespace DB_Talk.DAL 
{
	...
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_Departments  ");
			if(strWhere.Trim()!="")
			{
		...
		public int Add(DB_Talk.Model.m_Departments model,bool IsReturnID)   
		{
            ...
            strSql.Append("insert into m_Departments(");	
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Departments set ");
			if(model.vc_Name!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_Departments ");	
			else
				strSql.Append("update m_Departments  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
