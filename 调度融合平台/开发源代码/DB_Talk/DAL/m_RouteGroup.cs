...
namespace DB_Talk.DAL 
{
	...
	///数据访问类 m_RouteGroup
	/// </summary>	
	public partial class m_RouteGroup
	{ 
		public m_RouteGroup()
		{}
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_RouteGroup  ");
			if(strWhere.Trim()!="")
			{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_Talk.Model.m_RouteGroup model,bool IsReturnID)   
		{
		    int Result=0;
		    StringBuilder strSql=new StringBuilder();
            ...
            strSql.Append("insert into m_RouteGroup(");	
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_Talk.Model.m_RouteGroup model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_RouteGroup set ");
			if(model.BoxID!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_RouteGroup ");	
			else
				strSql.Append("update m_RouteGroup  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_RouteGroup GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, BoxID, vc_Code, vc_Name, vc_Memo, i_Flag  ");			
			strSql.Append("  from m_RouteGroup ");
			 strSql.Append(" where ID= '"+ ID +"'");   	  
			DB_Talk.Model.m_RouteGroup model=new DB_Talk.Model.m_RouteGroup();
			DataSet ds=GetDataSet(strSql.ToString());
			if(ds!=null && ds.Tables[0].Rows.Count>0)
			{
		...
		/// 根据条件得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_RouteGroup GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, BoxID, vc_Code, vc_Name, vc_Memo, i_Flag  ");			
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_Talk.Model.m_RouteGroup> DataTableToList(DataSet ds)
		{
			List<DB_Talk.Model.m_RouteGroup> modelList = new List<DB_Talk.Model.m_RouteGroup>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_Talk.Model.m_RouteGroup model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_Talk.Model.m_RouteGroup();	
