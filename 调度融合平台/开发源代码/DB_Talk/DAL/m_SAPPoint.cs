...
namespace DB_Talk.DAL 
{
	...
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_SAPPoint  ");
			if(strWhere.Trim()!="")
			{
		...
		public int Add(DB_Talk.Model.m_SAPPoint model,bool IsReturnID)   
		{
            ...
            strSql.Append("insert into m_SAPPoint(");	
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_SAPPoint set ");
			if(model.BoxID!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_SAPPoint ");	
			else
				strSql.Append("update m_SAPPoint  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
