...
namespace DB_Talk.DAL 
{
	...
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_AlarmInfo  ");
			if(strWhere.Trim()!="")
			{
		...
		public DB_Talk.Model.m_AlarmInfo GetModel(int ID)
		{
			...
			strSql.Append("  from m_AlarmInfo ");
			strSql.Append("  where ID='"+ID+"'");   	  
			DB_Talk.Model.m_AlarmInfo model=new DB_Talk.Model.m_AlarmInfo();
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_Talk.Model.m_AlarmInfo> DataTableToList(DataSet ds)
		{
			List<DB_Talk.Model.m_AlarmInfo> modelList = new List<DB_Talk.Model.m_AlarmInfo>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_Talk.Model.m_AlarmInfo model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_Talk.Model.m_AlarmInfo();	
