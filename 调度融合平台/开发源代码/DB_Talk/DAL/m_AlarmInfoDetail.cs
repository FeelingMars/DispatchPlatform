...
{
	/// <summary>
	///数据访问类 m_AlarmInfoDetail
	/// </summary>	
	public partial class m_AlarmInfoDetail
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_AlarmInfoDetail  ");
			if(strWhere.Trim()!="")
			{
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_AlarmInfoDetail GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Memo, i_Flag  ");			
			strSql.Append("  from m_AlarmInfoDetail ");
			strSql.Append("  where ID='"+ID+"'");   	  
			DB_Talk.Model.m_AlarmInfoDetail model=new DB_Talk.Model.m_AlarmInfoDetail();
		...
		/// 根据条件得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_AlarmInfoDetail GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Memo, i_Flag  ");			
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_Talk.Model.m_AlarmInfoDetail> DataTableToList(DataSet ds)
		{
			List<DB_Talk.Model.m_AlarmInfoDetail> modelList = new List<DB_Talk.Model.m_AlarmInfoDetail>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_Talk.Model.m_AlarmInfoDetail model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_Talk.Model.m_AlarmInfoDetail();	
