...
namespace DB_Talk.DAL
{
	...
	public partial class m_CameraInfo
	{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from m_CameraInfo ");
		...
		public DB_Talk.Model.m_CameraInfo GetModel(int ID)
		{
			...
			strSql.Append(" ID,BoxID,vc_Name,vc_Memo,i_ChanelID ");
			strSql.Append(" from m_CameraInfo ");
			strSql.Append(" where ID="+ID+"" );
			DB_Talk.Model.m_CameraInfo model=new DB_Talk.Model.m_CameraInfo();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["i_ChanelID"]!=null && ds.Tables[0].Rows[0]["i_ChanelID"].ToString()!="")
				{
					model.i_ChanelID=int.Parse(ds.Tables[0].Rows[0]["i_ChanelID"].ToString());
				}
				return model;
			}
			else
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BoxID,vc_Name,vc_Memo,i_ChanelID ");
			strSql.Append(" FROM m_CameraInfo ");
			if(strWhere.Trim()!="")
			{
