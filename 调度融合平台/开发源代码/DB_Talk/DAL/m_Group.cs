...
namespace DB_Talk.DAL 
{
	...
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_Group  ");
			if(strWhere.Trim()!="")
			{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_Talk.Model.m_Group model,bool IsReturnID)   
		{
		    int Result=0;
		    StringBuilder strSql=new StringBuilder();
		    ...
		    {
			   strSql1.Append("GroupTypeID,");	
			   strSql2.Append("'"+ model.GroupTypeID +"',");	
			} 
            ...
            strSql.Append("insert into m_Group(");	
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_Talk.Model.m_Group model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Group set ");
			if(model.BoxID!=null)
		    {
			...
			if(model.GroupTypeID!=null)
		    {
			   strSql.Append("GroupTypeID='"+ model.GroupTypeID +"',");	
			} 
			if(model.vc_Name!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_Group ");	
			else
				strSql.Append("update m_Group  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_Group GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
