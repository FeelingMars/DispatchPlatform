...
namespace DB_TeleBill.DAL 
{
	...
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_Role  ");
			if(strWhere.Trim()!="")
			{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_TeleBill.Model.m_Role model,bool IsReturnID)   
		{
		    int Result=0;
		    StringBuilder strSql=new StringBuilder();
            ...
            strSql.Append("insert into m_Role(");	
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_TeleBill.Model.m_Role model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Role set ");
			if(model.vc_Name!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_Role ");	
			else
				strSql.Append("update m_Role  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_TeleBill.Model.m_Role GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Memo, i_Flag  ");			
			strSql.Append("  from m_Role ");
			 strSql.Append(" where ID= '"+ ID +"'");   	  
			DB_TeleBill.Model.m_Role model=new DB_TeleBill.Model.m_Role();
			DataSet ds=GetDataSet(strSql.ToString());
			if(ds!=null && ds.Tables[0].Rows.Count>0)
			{
		...
		/// 根据条件得到一个对象实体
		/// </summary>
		public DB_TeleBill.Model.m_Role GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Memo, i_Flag  ");			
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_TeleBill.Model.m_Role> DataTableToList(DataSet ds)
		{
			List<DB_TeleBill.Model.m_Role> modelList = new List<DB_TeleBill.Model.m_Role>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_TeleBill.Model.m_Role model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_TeleBill.Model.m_Role();	
