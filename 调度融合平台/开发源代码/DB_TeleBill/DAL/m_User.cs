...
namespace DB_TeleBill.DAL 
{
	...
	{ 
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_TeleBill.Model.m_User model,bool IsReturnID)   
		{
		    int Result=0;
		    StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_TeleBill.Model.m_User model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
		...
		{	
			...
			if(IsTrueDelete)
				...
				strSql.Append("update m_User  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_TeleBill.Model.m_User GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Password, roleID, dt_CreateTime, vc_Memo, i_Flag  ");			
			strSql.Append("  from m_User ");
			 strSql.Append(" where ID= '"+ ID +"'");   	  
			DB_TeleBill.Model.m_User model=new DB_TeleBill.Model.m_User();
			DataSet ds=GetDataSet(strSql.ToString());
			if(ds!=null && ds.Tables[0].Rows.Count>0)
			{
		...
		/// 根据条件得到一个对象实体
		/// </summary>
		public DB_TeleBill.Model.m_User GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, vc_Name, vc_Password, roleID, dt_CreateTime, vc_Memo, i_Flag  ");			
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_TeleBill.Model.m_User> DataTableToList(DataSet ds)
		{
			List<DB_TeleBill.Model.m_User> modelList = new List<DB_TeleBill.Model.m_User>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_TeleBill.Model.m_User model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_TeleBill.Model.m_User();	
