...
namespace DispatchPlatform
{
    public partial class FormMain : Form
    {
        ...
        #region Video相关事件
        void _popVideoForm_OnMin(object sender, EventArgs e)
        {
            panelVideoBox.Controls.Add((System.Windows.Forms.Control)sender);
        }
        ...
        /// <param name="e"></param>
        void panelVideo_DoubleClick(object sender, EventArgs e)
        {
            _popVideoForm.ShowVideo(this.panelVideo, lblVideoNumber.Text);
            _popVideoForm.Show();
        }
        #endregion
        ...
        public void single_Click(object sender, EventArgs e)
        {
            ...
            switch (Pub._talkControl.CurrentNormalCMD)
            {
                ...
                case PublicEnums.EnumNormalCmd.RecordOperate:
                    ...
                    break;
                case PublicEnums.EnumNormalCmd.VideoCall:
                    lblVideoNumber.Text = control.MemberName + control.DepartmentName + control.Number.ToString();
                    panelVideoBox.Tag = control.Number;
                    panelVideoBox.Visible = true;
                    HTPhoneSDK.htphone_call(control.Number.ToString());
                    break;
                default:
                    break;
            }
        ...
        void _talkControl_OnMemberStateChanged(object obj, TalkControl.UserStateArgs e)
        {
            ...
            this.Invoke(new EventHandler(delegate(object o, EventArgs ee)
            {
                ...
                if (cVideoNumber.Number == e.UserNumber)
                {
                    cVideoNumber.UserLineStatus = e.UserLineStatus;
                }
            ...
            this.Invoke(new EventHandler(delegate(object o, EventArgs ee)
            {
                switch (cstate)
                {
                    ...
                    case HTPhoneSDK.HTphoneCallState.HTphoneCallOutgoingEarlyMedia:
                        break;
                    case HTPhoneSDK.HTphoneCallState.HTphoneCallConnected:
                        bc_OnMsg("视频呼叫成功");
                        lblVideoNumber.Text = _fromNumberForVideo + "来电";
                        panelVideoBox.Tag = _fromNumberForVideo;
                        panelVideoBox.Visible = true;
                        break;
        ...
        public void ShowLoadOk()
        {
            ...
            {
                int i = HTPhoneSDK.htphone_init(this.Handle.ToInt32(), panelVideo.Handle.ToInt32(), IntPtr.Zero, true);
                registerCallBack = new HTPhoneSDK.HTphoneRegistrationStateChangedCb(HTphoneRegistrationStateChangedCb);
                callStateCallBack = new HTPhoneSDK.HTphoneCallStateChangedCb(HTphoneCallStateChangedCb);
                HTPhoneSDK.htphone_set_callback(Marshal.GetFunctionPointerForDelegate(registerCallBack), Marshal.GetFunctionPointerForDelegate(callStateCallBack));

                bool pp = HTPhoneSDK.Register(_boxModel.vc_IP, Pub.VideoNumber, Pub.VideoPassword);
                HTPhoneSDK.VideoSizeEnum vs = (HTPhoneSDK.VideoSizeEnum)Pub._configModel.VideoSize;
                i = HTPhoneSDK.htphone_set_preferred_video_size(vs.GetHashCode());
                IntPtr local = HTPhoneSDK.htphone_get_video_device();
                string vv = Marshal.PtrToStringAnsi(local);
                panelVideo.DoubleClick += new EventHandler(panelVideo_DoubleClick);
                _popVideoForm.OnMin += new EventHandler(_popVideoForm_OnMin);

                #region //2014-11-11增加视频调度号

                cVideoNumber.Number = Convert.ToInt32(Pub.VideoNumber);
                cVideoNumber.TellType = PublicEnums.EnumTelType.WiFi手机;
                cVideoNumber.MemberName = "视频调度号码";
                cVideoNumber.UserLineStatus = TalkControl.EnumUserLineStatus.Offline;

                #endregion

                smallTimeControl2.Visible = true;
                btnVideoSize.Visible = true;
            }
            else
            {
                btnVideoSize.Visible = false;
                smallTimeControl2.Visible = false;
                panelVideoBox.Visible = false;
                cVideoNumber.Visible = false;
            }
            #endregion
