...
{
	/// <summary>
	///数据访问类 m_CallingSource
	/// </summary>	
	public partial class m_CallingSource
	{ 
		...
		public bool Exists(string strWhere)
		{
			...
			strSql.Append("select count(1) from m_CallingSource  ");
			if(strWhere.Trim()!="")
			{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_Talk.Model.m_CallingSource model,bool IsReturnID)   
		{
		    int Result=0;
		    StringBuilder strSql=new StringBuilder();
            ...
            if(model.i_MainType!=null)
		    {
			   strSql1.Append("i_MainType,");	
			   strSql2.Append("'"+ model.i_MainType +"',");	
			} 
            if(model.i_SubType!=null)
		    {
			   strSql1.Append("i_SubType,");	
			   strSql2.Append("'"+ model.i_SubType +"',");	
			} 
            ...
            strSql.Append("insert into m_CallingSource(");	
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_Talk.Model.m_CallingSource model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_CallingSource set ");
			if(model.BoxID!=null)
		    {
			...
			if(model.i_MainType!=null)
		    {
			   strSql.Append("i_MainType='"+ model.i_MainType +"',");	
			} 
			if(model.i_SubType!=null)
		    {
			   strSql.Append("i_SubType='"+ model.i_SubType +"',");	
			} 
			if(model.vc_Memo!=null)
		    {
		...
		{	
			StringBuilder strSql=new StringBuilder();
			if(IsTrueDelete)
			    strSql.Append("delete from m_CallingSource ");	
			else
				strSql.Append("update m_CallingSource  set i_Flag=1 ");	 
		    strSql.Append(" where ");
		    strSql.Append("ID=" + ID +"");	  
			int rows=ExecuteSql(strSql.ToString());
			if (rows > 0)
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_CallingSource GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, BoxID, vc_Code, vc_Name, i_MainType, i_SubType, vc_Memo, i_Flag  ");			
			strSql.Append("  from m_CallingSource ");
			 strSql.Append(" where ID= '"+ ID +"'");   	  
			DB_Talk.Model.m_CallingSource model=new DB_Talk.Model.m_CallingSource();
			DataSet ds=GetDataSet(strSql.ToString());
			if(ds!=null && ds.Tables[0].Rows.Count>0)
			{
		...
		/// 根据条件得到一个对象实体
		/// </summary>
		public DB_Talk.Model.m_CallingSource GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("  select top 1 ");
			strSql.Append("  ID, BoxID, vc_Code, vc_Name, i_MainType, i_SubType, vc_Memo, i_Flag  ");			
		...
		/// 获得数据列表
		/// </summary>
		public List<DB_Talk.Model.m_CallingSource> DataTableToList(DataSet ds)
		{
			List<DB_Talk.Model.m_CallingSource> modelList = new List<DB_Talk.Model.m_CallingSource>();
			if (ds == null) return modelList;
            DataTable dt = ds.Tables[0];
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DB_Talk.Model.m_CallingSource model;
				for (int n = 0; n < rowsCount; n++)
				{
				  model = new DB_Talk.Model.m_CallingSource();	
				  ...
				  if(dt.Rows[n]["i_MainType"]!=null && dt.Rows[n]["i_MainType"].ToString()!="")
				  {
				      model.i_MainType=int.Parse(dt.Rows[n]["i_MainType"].ToString());
				  }
				  if(dt.Rows[n]["i_SubType"]!=null && dt.Rows[n]["i_SubType"].ToString()!="")
				  {
				      model.i_SubType=int.Parse(dt.Rows[n]["i_SubType"].ToString());
				  }
